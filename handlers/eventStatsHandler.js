...
class EventStatsHandler {
 /**
  * Cria e envia o painel de estatísticas (Wrapper para compatibilidade)
  * @param {TextChannel} channel - Canal onde enviar
  * @param {Guild} guild - Objeto da guilda
  */
 static async sendPanel(channel, guild) {
  try {
   console.log(`[EventStatsHandler] Wrapper sendPanel chamado, redirecionando para createAndSendPanel`);
   return await this.createAndSendPanel(channel, guild);
  } catch (error) {
   console.error(`[EventStatsHandler] Erro no wrapper sendPanel:`, error);
   throw error;
  }
 }
