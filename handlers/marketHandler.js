/**
 * marketHandler.js - Sistema de Pesquisa de Preços do Albion Online
 * 
 * VERSÃO CORRIGIDA - Importação e Timezone UTC
 * 
 * CORREÇÕES CRÍTICAS:
 * 1. CORREÇÃO CASE-SENSITIVE: Importação agora usa './albionMarketApi.js' (camelCase correto para Linux/Replit)
 * 2. Timezone UTC: Todos os timestamps via Database.getCurrentTimestamp()
 * 3. Adicionado import do Database para auditoria e consistência
 */
...
// CORREÇÃO CRÍTICA: Nome do arquivo deve estar em camelCase exato (Linux case-sensitive)
// Antes: require('./albionMarketApi') - funcionava apenas no Windows
// Agora: require('./albionMarketApi.js') - funciona em Linux/Replit (case-sensitive)
const MarketApi = require('./albionMarketApi.js');
const Database = require('./database.js');

/**
 * MarketHandler - Sistema de Pesquisa de Preços do Albion Online
 * Sistema de navegação por menus igual ao ORB XP
 * VERSÃO UTC - Timezone consistente
 */
class MarketHandler {
  constructor() {
    ...
    console.log('[MarketHandler] Inicializado com timezone UTC');
  }

  /**
   * Retorna timestamp UTC atual via Database
   */
  getTimestamp() {
  ...
  async handleBrowseCategory(interaction) {
    try {
      const searchId = `${interaction.user.id}_${this.getTimestamp()}`;
  ...
  async executeSearch(interaction, searchId) {
    try {
      ...
      // Log de auditoria
      await Database.logAudit(interaction.guild.id, 'MARKET_SEARCH', interaction.user.id, {
        item: searchData.item.name,
        tier: searchData.tier,
        enchant: searchData.enchant,
  ...
  async handleUpdateCache(interaction) {
    try {
      ...
      // Log de auditoria
      await Database.logAudit(interaction.guild.id, 'MARKET_CACHE_UPDATE', interaction.user.id, {
        timestamp: this.getTimestamp(),
        itemsCount: MarketApi.itemsCache.size
      });
  ...
  cleanupExpiredSessions() {
    ...
    for (const [searchId, data] of this.activeSearches) {
    ...
    if (cleaned > 0) {
      console.log(`[MarketHandler] ${cleaned} sessões expiradas limpas`);
    }
  }
}
