...
class RegistrationActions {
  static initialize() {
    ...
    // Map para controlar registros em processamento (evita duplo clique)
    this.processingRegistrations = new Map();
  }

  /**
   * Verifica se um usuário ou nick está na blacklist (AGORA VIA BANCO DE DADOS)
   ...
   */
  static async checkBlacklist(nick, userId) {
    try {
      // Verificar blacklist por userId no banco de dados global
      const blacklisted = await Database.isBlacklisted(userId);
      if (blacklisted) {
        return {
          isBlacklisted: true,
          reason: blacklisted.motivo || 'Motivo não especificado'
        };
      }

      // Verificar blacklist por nick (case insensitive) - busca em todos os registros
      const allBlacklist = await Database.getBlacklist();
      for (const entry of allBlacklist) {
        if (entry.nick && entry.nick.toLowerCase() === nick.toLowerCase()) {
          return {
            isBlacklisted: true,
            reason: entry.motivo || 'Motivo não especificado'
          };
        }
      }
      ...
      console.error('[RegistrationActions] Erro ao verificar blacklist:', error);
  ...
  /**
   * Retorna histórico de recusas de um usuário (AGORA VIA BANCO DE DADOS)
   * @param {string} userId - ID do usuário
   * @param {string} nick - Nick do jogador
   * @returns {Array} - Lista de recusas anteriores
   */
  static async getHistoricoRecusas(userId, nick) {
    try {
      // Buscar histórico do banco de dados (audit_logs)
      const logs = await Database.getAuditLogs(null, {
        actionType: 'REGISTRO_RECUSADO',
        userId: userId
      }, 10);

      return logs.map(log => ({
        tipo: 'recusado',
        motivo: log.details?.motivo || 'Motivo não especificado',
        nick: log.details?.nick || nick,
        data: log.created_at,
        analisadoPor: log.details?.analisadoPor || 'Sistema'
      }));
    } catch (error) {
      console.error('[RegistrationActions] Erro ao buscar histórico:', error);
      return [];
    }
  }
  ...
  // Verifica se o registro já está sendo processado
  static isProcessing(regId) {
    return this.processingRegistrations?.has(regId);
  }

  // Marca registro como processando
  ...
  static async approveAsMember(interaction, regId) {
    try {
      ...
      // Se registro não existe, verifica se já foi processado anteriormente
      if (!registro) {
        return interaction.reply({
      ...
      // Deleta a mensagem do canal de solicitação
      try {
      ...
      // Garante remoção do processamento em caso de erro
      this.processingRegistrations?.delete(regId);
  ...
  static async approveAsConvidado(interaction, regId) {
    try {
      ...
      // Deleta a mensagem do canal de solicitação ao aprovar
      try {
        await interaction.message.delete();
        console.log(`[Registration] Mensagem de registro ${regId} deletada após aprovação`);
      } catch (e) {
        console.log('[Registration] Não foi possível deletar mensagem de convidado:', e);
        await interaction.update({
  ...
  static async processRejectionWithReason(interaction, regId) {
    try {
      ...
      // Salvar no histórico de recusas (AGORA NO BANCO DE DADOS)
      await Database.logAudit(interaction.guild.id, 'REGISTRO_RECUSADO', registro.userId, {
        motivo: motivo,
        nick: nick,
      ...
      // Deleta a mensagem do canal de solicitação ao recusar
      try {
        await interaction.message.delete();
  ...
  static async processBlacklistAdd(interaction, regId) {
    try {
      ...
      // Adicionar à blacklist NO BANCO DE DADOS GLOBAL
      await Database.addToBlacklist(registro.userId, {
        nick: nick,
        guilda: guilda,
        motivo: motivo,
      ...
      // Deleta a mensagem do canal de solicitação ao adicionar na blacklist
      try {
        await interaction.message.delete();
  ...
  static async checkExistingRegistration(guild, userId, nick) {
    ...
    const blacklistCheck = await this.checkBlacklist(nick, userId);
    if (blacklistCheck.isBlacklisted) {
      erros.push(`🚫 Você está na blacklist! Motivo: ${blacklistCheck.reason}`);
    }
