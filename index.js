...
async function handleButton(interaction, customId) {
 try {
  ...
  // BOTÃO ATUALIZAR BOT - SINCRONIZAÇÃO COMPLETA
  if (customId === 'config_atualizar_bot') {
   try {
    ...
    const resultado = await setup.syncServer();

    // Cria embed de resultado
    const embed = new EmbedBuilder()
     .setTitle('🔄 SINCRONIZAÇÃO CONCLUÍDA')
     .setDescription('O bot foi atualizado com sucesso!')
     .setColor(resultado.erros.length === 0 ? 0x2ECC71 : 0xF1C40F)
     .addFields(
      {
       name: '📁 Canais/Categorias Criados',
       value: resultado.canaisCriados.length > 0 
        ? resultado.canaisCriados.join('\n').substring(0, 1024)
        : 'Nenhum (todos já existiam)',
       inline: false
      },
      {
       name: '🎭 Cargos Criados',
       value: resultado.cargosCriados.length > 0
        ? resultado.cargosCriados.map(r => `\`@${r}\``).join(', ').substring(0, 1024)
        : 'Nenhum (todos já existiam)',
       inline: true
      },
      {
       name: '🎨 Painéis Atualizados',
       value: resultado.paineisAtualizados.length > 0
        ? resultado.paineisAtualizados.map(p => `\`#${p}\``).join(', ').substring(0, 1024)
        : 'Nenhum',
       inline: true
      },
      {
       name: '⚡ Comandos Slash',
       value: resultado.comandosRegistrados ? '✅ Registrados' : '❌ Erro',
       inline: true
      },
      {
       name: '🔓 Botões Destravados',
       value: resultado.botoesDestravados.length > 0
        ? `${resultado.botoesDestravados.length} canais`
        : 'Nenhum travado',
       inline: true
      }
     )
     .setFooter({ 
      text: resultado.erros.length > 0 
       ? `⚠️ ${resultado.erros.length} erros encontrados (ver console)`
       : 'NOTAG Bot • Sistema atualizado'
     })
     .setTimestamp();

    // Se houver erros, adiciona field (limitado)
    if (resultado.erros.length > 0) {
     const errosText = resultado.erros.slice(0, 5).join('\n');
     embed.addFields({
      name: '⚠️ Erros (primeiros 5)',
      value: `\`\`\`${errosText}\`\`\``.substring(0, 1024),
      inline: false
     });
    }
    ...
    // Log no canal de auditoria se existir
    const auditChannel = interaction.guild.channels.cache.find(c => c.name.includes('log') || c.name.includes('auditoria'));
    if (auditChannel) {
     await auditChannel.send({
      embeds: [
       new EmbedBuilder()
        .setTitle('📝 LOG: Sincronização do Bot')
        .setDescription(`**Executado por:** <@${interaction.user.id}>\n**Data:** ${new Date().toLocaleString()}`)
        .setColor(0x3498DB)
        .addFields(
         { name: 'Canais Criados', value: `${resultado.canaisCriados.length}`, inline: true },
         { name: 'Cargos Criados', value: `${resultado.cargosCriados.length}`, inline: true },
         { name: 'Paineis Atualizados', value: `${resultado.paineisAtualizados.length}`, inline: true },
         { name: 'Erros', value: `${resultado.erros.length}`, inline: true }
        )
        .setTimestamp()
      ]
     });
    ...
    console.error('[Button:config_atualizar_bot] Error:', error);
    await interaction.editReply({
     content: '❌ Erro ao sincronizar bot. Verifique o console.',
     ephemeral: true
    });
   }
   return;
...
async function handleModal(interaction) {
 try {
  ...
  if (customId.startsWith('modal_recurrence_')) {
   const templateId = interaction.customId.replace('modal_recurrence_', '');
