...
client.on(Events.InteractionCreate, async interaction => {
  try {
      ...
      if (interaction.isButton()) {
          ...
          // 🛒 MERCADO ALBION - NOVO
          if (customId === 'market_search_item') {
              await MarketHandler.handleSearchButton(interaction);
              return;
          }

          if (customId === 'market_search_history') {
              await interaction.reply({
                  content: '📜 Histórico de pesquisas em desenvolvimento...',
                  ephemeral: true
              });
              return;
          }
          ...
          if (customId.startsWith('market_history_')) {
              const parts = customId.replace('market_history_', '').split('_');
              const itemId = parts.slice(0, -1).join('_');
              const quality = parts[parts.length - 1];
              await MarketHandler.showPriceHistory(interaction, itemId, quality);
              return;
          }
          ...
          if (customId.startsWith('market_search_cancel_')) {
              const searchId = customId.replace('market_search_cancel_', '');
      ...
      if (interaction.isStringSelectMenu()) {
          ...
          // 🛒 MERCADO - Select Menus
          if (interaction.customId === 'market_select_item') {
              const itemId = interaction.values[0];
              const items = Array.from(MarketApi.itemsCache.values());
              const item = items.find(i => i.id === itemId);
              if (item) {
                  await MarketHandler.showFiltersSelect(interaction, item);
              }
              return;
          }
