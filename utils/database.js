 ...
 * Todas as operações devem usar UTC internamente
 * Conversão para BRT (UTC-3) apenas na camada de apresentação (handlers/commands)
 */
const TIMEZONE = {
  UTC: 'UTC',
  BRAZIL: 'America/Sao_Paulo',
  OFFSET_BR: -3 * 60 * 60 * 1000 // -3 horas em milissegundos
};

/**
 * Database Manager - Versão Multi-Servidor (Timezone UTC Corrigido)
 * 
 * CORREÇÕES DE TIMEZONE:
 * 1. Todos os timestamps armazenados como INTEGER (milliseconds UTC)
 * 2. Função getCurrentTimestamp() garante consistência entre SQLite e JS
 * 3. Correção crítica: recordCheckin agora usa UTC para calcular "hoje"
 * 4. Validação de timestamps em todas as operações críticas
 * 5. Logs de debug para auditoria de timezone
 */
class DatabaseManager {
  ...
  async initialize() {
    try {
      ...
      console.log('[Database] Timezone padrão: UTC (Timestamp integer ms)');
  ...
  /**
   * Retorna timestamp UTC atual em milissegundos
   * Substitui Date.now() direto para permitir mocking/testes e logs
   * @returns {number} Timestamp UTC em ms
   */
  getCurrentTimestamp() {
    const now = Date.now();
    // Validação: garante que estamos retornando um número válido
    if (typeof now !== 'number' || isNaN(now)) {
      console.error('[Database][CRITICAL] Date.now() retornou valor inválido:', now);
      throw new Error('Timestamp inválido gerado pelo sistema');
    }
    return now;
  }

  /**
   * Retorna data atual em formato YYYY-MM-DD baseado em UTC
   * CRITICAL FIX: Substitui new Date().toISOString() que causava bug no Brasil (UTC-3)
   * Quando o servidor está em UTC-3, new Date().toISOString() pode retornar o dia anterior
   * entre 00:00 e 03:00 da manhã.
   * 
   * @returns {string} Data formatada YYYY-MM-DD (UTC)
   */
  getCurrentDateUTC() {
  ...
  /**
   * Valida se um timestamp está em um range razoável (2020-2030)
   * Útil para detectar erros de timezone ou unidades (segundos vs ms)
   */
  isValidTimestamp(timestamp) {
    if (typeof timestamp !== 'number' || isNaN(timestamp)) return false;
    const min = 1577836800000; // 2020-01-01T00:00:00.000Z
    const max = 1893456000000; // 2030-01-01T00:00:00.000Z
    return timestamp >= min && timestamp <= max;
  }

  /**
   * Converte timestamp para string ISO UTC para logs
   */
  timestampToISO(timestamp) {
    if (!this.isValidTimestamp(timestamp)) return 'invalid_timestamp';
  ...
  async createTables(db, isGlobal = false) {
    const now = this.getCurrentTimestamp();
    console.log(`[Database] Criando tabelas (timestamp ref: ${this.timestampToISO(now)})`);
    ...
    // Tabela de checkins diários - CORRIGIDO: date continua TEXT (YYYY-MM-DD), mas será UTC
    await db.runAsync(`
    ...
    // Migração: corrigir timestamps antigos que podem estar em segundos ou com offset errado
    await this.migrateOldTimestamps(db);
  }

  /**
   * Migração: corrige timestamps que podem estar em segundos ao invés de ms
   * ou que foram salvos com offset de timezone incorreto
   */
  async migrateOldTimestamps(db) {
    try {
      const tables = ['users', 'transactions', 'events', 'audit_logs', 'votes'];

      for (const table of tables) {
        // Corrige timestamps em segundos (menores que 10000000000 são provavelmente segundos)
        await db.runAsync(`
          UPDATE ${table} 
          SET created_at = created_at * 1000 
          WHERE created_at < 10000000000 AND created_at > 0
        `).catch(() => {}); // Ignora erro se tabela não existir ou não ter dados

        await db.runAsync(`
          UPDATE ${table} 
          SET updated_at = updated_at * 1000 
          WHERE updated_at < 10000000000 AND updated_at > 0
        `).catch(() => {});
      }

      console.log('[Database] Migração de timestamps concluída');
    } catch (error) {
      console.error('[Database] Erro na migração de timestamps:', error);
    }
  }
  ...
  async registerGuild(guildId, guildName, ownerId) {
    ...
    try {
      await this.defaultDb.runAsync(`
        INSERT OR REPLACE INTO guild_registry (guild_id, guild_name, owner_id, added_at, last_activity)
        VALUES (?, ?, ?, ?, ?)
      `, [guildId, guildName, ownerId, now, now]);

      console.log(`[Database] Guild registrada: ${guildId} em ${this.timestampToISO(now)}`);

      // Criar banco do servidor se não existir
      await this.getGuildDb(guildId);
    } catch (error) {
      console.error(`[Database] Erro ao registrar guild ${guildId}:`, error);
      throw error;
    }
  }
  ...
  async updateGuildActivity(guildId) {
    ...
    try {
      await this.defaultDb.runAsync(`
        UPDATE guild_registry SET last_activity = ? WHERE guild_id = ?
      `, [now, guildId]);
    } catch (error) {
      console.error(`[Database] Erro ao atualizar atividade da guild ${guildId}:`, error);
    }
  }
  ...
  async getGuildList() {
    ...
    return rows.map(row => ({
      ...row,
      added_at_iso: this.timestampToISO(row.added_at),
      last_activity_iso: this.timestampToISO(row.last_activity)
    }));
  }
    ...
    try {
      ...
      return {
        ...
        guildaRegistrada: row.guilda_nome ? {
          ...
          dataRegistroISO: this.timestampToISO(row.guilda_registrada_em)
        } : null,
        ...
        updatedAtISO: this.timestampToISO(row.updated_at)
      };
    } catch (error) {
    ...
    try {
      ...
      if (data.guildaRegistrada !== undefined) {
        if (data.guildaRegistrada === null) {
          ...
          const regTimestamp = data.guildaRegistrada.dataRegistro || now;
          if (!this.isValidTimestamp(regTimestamp)) {
            console.warn(`[Database] Timestamp de registro inválido para guild ${guildId}: ${regTimestamp}, usando now()`);
            values.push(now);
          } else {
            values.push(regTimestamp);
          }
        }
      }
      ...
      console.log(`[Database] Guild config updated for ${guildId} at ${this.timestampToISO(now)}`);
    } catch (error) {
  ...
  async getUser(guildId, userId) {
    ...
    if (!row) {
      await db.runAsync(
        'INSERT INTO users (user_id, ultimo_login, created_at, updated_at) VALUES (?, ?, ?, ?)',
        [userId, now, now, now]
      );
  ...
  parseUser(row) {
    ...
    return {
      ...
      ultimoCheckinISO: this.timestampToISO(row.ultimo_checkin),
      createdAt: row.created_at,
      createdAtISO: this.timestampToISO(row.created_at),
      updatedAt: row.updated_at,
      updatedAtISO: this.timestampToISO(row.updated_at)
    };
  }
  ...
  async addTransaction(guildId, transaction) {
    ...
    const id = transaction.id || `txn_${now}_${Math.random().toString(36).substr(2, 9)}`;

    // Validação do approved_at se existir
    let approvedAt = transaction.approvedAt || null;
    if (approvedAt && !this.isValidTimestamp(approvedAt)) {
      console.warn(`[Database] approvedAt inválido na transação: ${approvedAt}`);
      approvedAt = now;
    }

    await db.runAsync(`
      INSERT INTO transactions
      (id, type, user_id, amount, reason, event_id, approved_by, approved_at, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      id,
      transaction.type,
    ...
    console.log(`[Database] Transação ${id} criada em ${this.timestampToISO(now)}`);
  }
  ...
  async getUserTransactions(guildId, userId, limit = 50) {
    ...
    return rows.map(row => ({
      ...row,
      createdAtISO: this.timestampToISO(row.created_at),
      approvedAtISO: this.timestampToISO(row.approved_at)
    }));
  }
  ...
  async getEventsByPeriod(guildId, days) {
    ...
    try {
      const now = this.getCurrentTimestamp();
      const since = days === 0 ? 0 : now - (days * 24 * 60 * 60 * 1000);
      ...
      return events.map(event => ({
        ...event,
        createdAtISO: this.timestampToISO(event.created_at),
        startedAtISO: this.timestampToISO(event.started_at),
        endedAtISO: this.timestampToISO(event.ended_at)
      }));
    } catch (error) {
  ...
  async addEventHistory(guildId, historyData) {
    ...
    try {
      // Validação do timestamp recebido
      let timestamp = historyData.timestamp;
      if (!this.isValidTimestamp(timestamp)) {
        console.warn(`[Database] Timestamp inválido em historyData: ${timestamp}, usando now()`);
        timestamp = now;
      }
      ...
      console.log(`[Database] Event history added for ${guildId} at ${this.timestampToISO(timestamp)}`);
    } catch (error) {
  ...
  async getEventHistory(guildId, limit = 50) {
    ...
    try {
      ...
      return rows.map(row => ({
        ...row,
        timestampISO: this.timestampToISO(row.timestamp),
  ...
  async logAudit(guildId, actionType, userId, details = {}, targetId = null) {
    ...
    await db.runAsync(`
      INSERT INTO audit_logs (action_type, user_id, target_id, details, created_at)
      VALUES (?, ?, ?, ?, ?)
    `, [
      actionType,
      userId,
    ...
    console.log(`[Audit] ${actionType} por ${userId} em ${guildId} em ${this.timestampToISO(now)}`);
  }
  ...
  async saveEvent(guildId, eventData) {
    ...
    // Validação de timestamps do evento
    let inicioTimestamp = eventData.inicioTimestamp || null;
    let finalizadoEm = eventData.finalizadoEm || null;

    if (inicioTimestamp && !this.isValidTimestamp(inicioTimestamp)) {
      console.warn(`[Database] inicioTimestamp inválido: ${inicioTimestamp}`);
      inicioTimestamp = null;
    }

    if (finalizadoEm && !this.isValidTimestamp(finalizadoEm)) {
      console.warn(`[Database] finalizadoEm inválido: ${finalizadoEm}`);
      finalizadoEm = null;
    }
    ...
    console.log(`[Database] Evento ${eventData.id} salvo (inicio: ${this.timestampToISO(inicioTimestamp)}, fim: ${this.timestampToISO(finalizadoEm)})`);
  }
  ...
  async getEventHistoryTable(guildId, limit = 50) {
    ...
    return rows.map(row => ({
      ...row,
      createdAtISO: this.timestampToISO(row.created_at),
      startedAtISO: this.timestampToISO(row.started_at),
      endedAtISO: this.timestampToISO(row.ended_at)
    }));
  }

  // ==================== DAILY CHECKINS (POR SERVIDOR) ====================
  // CRITICAL FIX: Corrigido bug de timezone no cálculo da data "hoje"

  async recordCheckin(guildId, userId, rewards) {
    const db = await this.getGuildDb(guildId);
    const now = this.getCurrentTimestamp();

    // CRITICAL FIX: Usa UTC para determinar "hoje" ao invés de new Date().toISOString()
    // que causava bug no Brasil (UTC-3) retornando o dia anterior entre 00:00-03:00
    const today = this.getCurrentDateUTC();

    console.log(`[Database] RecordCheckin: User ${userId}, Date (UTC): ${today}, Timestamp: ${this.timestampToISO(now)}`);
    ...
    console.log(`[Database] Checkin recorded for ${userId} on ${today} (UTC)`);

    return { ...rewards, date: today, timestamp: now };
  }

  async getTodayCheckin(guildId, userId) {
    const db = await this.getGuildDb(guildId);

    // CRITICAL FIX: Usa UTC para determinar "hoje"
    const today = this.getCurrentDateUTC();
    ...
    if (row) {
      console.log(`[Database] Found checkin for ${userId} on ${today}`);
    }
  ...
  /**
   * Retorna a data do último checkin do usuário (para cálculo de streak)
   */
  async getLastCheckinDate(guildId, userId) {
    const db = await this.getGuildDb(guildId);
    const row = await db.getAsync(`
  ...
  async saveTemplate(guildId, creatorId, template) {
    ...
    const result = await db.runAsync(`
      INSERT INTO event_templates (creator_id, name, description, requirements, default_duration, recurrence_rule, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [
      creatorId,
      template.name,
  ...
  async getTemplates(guildId) {
    ...
    return rows.map(t => ({
      ...t,
      createdAtISO: this.timestampToISO(t.created_at),
  ...
  async createVote(guildId, voteData) {
    ...
    // Validação do endsAt
    let endsAt = voteData.endsAt;
    if (!this.isValidTimestamp(endsAt)) {
      console.error(`[Database] endsAt inválido para votação: ${endsAt}`);
      throw new Error('Timestamp de término inválido');
    }

    // Garante que endsAt é no futuro
    if (endsAt <= now) {
      console.error(`[Database] endsAt deve ser no futuro: ${endsAt} <= ${now}`);
      throw new Error('Data de término deve ser no futuro');
    }

    await db.runAsync(`
      INSERT INTO votes (vote_id, creator_id, title, description, options, ends_at, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [
      voteData.id,
      voteData.creatorId,
    ...
    console.log(`[Database] Votação ${voteData.id} criada, termina em ${this.timestampToISO(endsAt)}`);
  }
  ...
  async getVote(guildId, voteId) {
    ...
    if (row) {
      ...
      row.endsAtISO = this.timestampToISO(row.ends_at);
      row.createdAtISO = this.timestampToISO(row.created_at);
    }
    return row;
  }
  ...
  async castVote(guildId, voteId, userId, optionIndex) {
    ...
    if (!vote || vote.status !== 'active') {
      console.log(`[Database] Voto rejeitado: votação ${voteId} não existe ou não está ativa`);
      return false;
    }

    if (now > vote.ends_at) {
      console.log(`[Database] Voto rejeitado: votação ${voteId} já expirou`);
      return false;
    }

    const votes = JSON.parse(vote.votes);
    if (votes[userId] !== undefined) {
      console.log(`[Database] Voto rejeitado: usuário ${userId} já votou`);
      return false;
    }
    ...
    console.log(`[Database] Voto registrado: ${userId} na opção ${optionIndex} da votação ${voteId}`);
    return true;
  }
  ...
  async addToBlacklist(userId, data) {
    ...
    await this.defaultDb.runAsync(`
      INSERT OR REPLACE INTO blacklist (user_id, nick, guilda, motivo, added_by, guild_id, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [userId, data.nick, data.guilda, data.motivo, data.addedBy, data.guildId, now]);

    console.log(`[Database] Usuário ${userId} adicionado à blacklist em ${this.timestampToISO(now)}`);
  }
  ...
  async getBlacklist() {
    const rows = await this.defaultDb.allAsync('SELECT * FROM blacklist ORDER BY created_at DESC');
    return rows.map(row => ({
      ...row,
      createdAtISO: this.timestampToISO(row.created_at)
    }));
  ...
  async removeFromBlacklist(userId) {
    ...
    console.log(`[Database] Usuário ${userId} removido da blacklist`);
  }
  ...
  async cleanup() {
    ...
    for (const [guildId, db] of this.databases) {
      try {
        ...
        const eventsDeleted = await db.runAsync('DELETE FROM events WHERE ended_at < ?', [ninetyDaysAgo]);
        const auditDeleted = await db.runAsync('DELETE FROM audit_logs WHERE created_at < ?', [ninetyDaysAgo]);

        console.log(`[Database] Cleanup completed for guild: ${guildId} (Events: ${eventsDeleted.changes}, Audit: ${auditDeleted.changes})`);
      } catch (e) {
  ...
  close() {
    console.log('[Database] Fechando todas as conexões...');
    if (this.defaultDb) {
